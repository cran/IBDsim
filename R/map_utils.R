...
function(Mb=NULL, cM=NULL, M=NULL, cm.per.mb=1, chromosome=1) { #genL numeric of length 1 or 2: genetic length male & female
	...
	male_phys = as.numeric(map$male[2,1])
	if(male_phys != map$female[2,1]) stop("Male and female chromosomes must have equal physical length.")  
	structure(map, length_Mb = male_phys, chromosome = chromosome, class = "chromosomeMap")
}
 
loadMap <-
function(map, chrom=1:23) {
  ...
  if (is.character(map)) {
	...
	if(length(maps)==2 && is.matrix(maps[[2]]))   maps=list(maps)
